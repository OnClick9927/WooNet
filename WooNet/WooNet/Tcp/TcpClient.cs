...
namespace WooNet
{
    ...
    public class TcpClient : TcpSocket, IDisposable, ITcpClient
    {
        #region variable
        ...
        private ChannelType channelProviderState = ChannelType.Async;
        private LockParam lParam = new LockParam();
        private ArgPool sendTokenManager = null;
        private BytePool sBufferManager = null;
        ...
        public TcpClient(int chunkBufferSize = 4096, int bufferNumber = 8)
        ...
        public void Connect(int port, string ip)
        {
            try
            {
                ...
                isConnected = false;
                channelProviderState = ChannelType.Async;
        ...
        public bool ConnectTo(int port, string ip)
        {
            try
            {
                ...
                isConnected = false;
                channelProviderState = ChannelType.AsyncWait;
        ...
        public bool ConnectSync(int port, string ip)
        {
            ...
            isConnected = false;
            channelProviderState = ChannelType.Sync;
            int retry = 3;
        ...
        public bool Send(Segment sendSegment, bool waiting = true)
        {
            try
            {
                if (IsClose())
        ...
        /// <returns></returns>
        public int SendSync(Segment sendSegment, Segment receiveSegment)
        {
            if (channelProviderState != ChannelType.Sync)
            {
        ...
        public void ReceiveSync(Segment receiveSegment, Action<Segment> receivedAction)
        {
            if (channelProviderState != ChannelType.Sync)
            {
                throw new Exception("需要使用同步连接...ConnectSync");
        ...
        private void InitializePool(int maxNumberOfConnections)
        {
            ...
            sendTokenManager = new ArgPool(cnt);
            sBufferManager = new BytePool(cnt, receiveChunkSize);
        ...
        private void ProcessConnectCallback(SocketAsyncEventArgs e)
        {
            try
            {
                ...
                if (channelProviderState == ChannelType.AsyncWait)
                    mReset.Set();
            }
            catch (Exception ex)
            {
